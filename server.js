...
async function initializeData() {
  try {
    ...
    console.log(
      "ℹ️ Categories will not be created automatically. Use admin panel to create categories.",
    );
    ...
    if (menuItemsCount === 0) {
      console.log(
        "ℹ️ No menu items found. Admin must add items through the admin panel.",
      );
    }
  ...
  try {
    ...
    if (updates.exchangeRate) {
      ...
      if (currentSettings && currentSettings.exchangeRate !== newRate) {
        ...
        console.log(
          `⏳ Rate Change Scheduled: ${newRate} to apply at ${nextMidnight}`,
        );
      }
    }
  ...
  try {
    ...
    const transformedMenu = menu.map((item) => {
      ...
      return {
        ...
        image:
          item.image ||
          "https://via.placeholder.com/200x130/F5F7FA/9E9E9E?text=No+Image",
      };
    });
...
app.get("/api/invoices/:id", async (req, res) => {
  try {
    ...
    let invoice = isObjectId
      ...
      : await Invoice.findOne({ invoiceId: id }).lean();
...
app.post("/api/invoices", async (req, res) => {
  try {
    ...
    sendInvoiceNotification(invoice).catch((err) =>
...
app.post("/api/admin/menu", async (req, res) => {
  try {
    ...
    if (!name || !categories || !originalPrice)
      return res
        .status(400)
        .json({ error: "Name, categories, and price are required" });
    ...
    if (invalidCategories.length > 0) {
      return res.status(400).json({
        error: `Categories "${invalidCategories.join(", ")}" do not exist.`,
      });
    }
    ...
    res
      .status(500)
      .json({ error: "Failed to create menu item", details: error.message });
  }
});
...
app.get("/api/admin/stats", async (req, res) => {
  try {
    ...
    const [totalInvoices, todayInvoices, totalMenuItems, totalUsers] =
      await Promise.all([
...
app.get("/", (req, res) => {
  res.send(
    "✅ POS Backend is Running! (Access the App via your GitHub Pages URL)",
  );
});
  ...
  try {
    ...
    if (settings && settings.pendingExchangeRate && settings.rateEffectiveAt) {
      ...
      if (now >= settings.rateEffectiveAt) {
        console.log(
          `🕛 12AM REACHED: Updating Exchange Rate from ${settings.exchangeRate} to ${settings.pendingExchangeRate}`,
        );
